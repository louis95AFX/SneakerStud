...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // The previous searchButton (open-search) logic is now handled by the mobileSearchButton in the navbar component.
    let searchInput; 
    let searchModal;
    ...
    function setupSearchModal() {
        ...
        // Search input functionality (for the modal search)
        searchInput.addEventListener('input', (e) => {
            handleSearch(e.target.value);
        });
    }
    ...
    // 🚀 NEW FUNCTION: Central search handler
    function handleSearch(query) {
        currentSearchTerm = query.toLowerCase().trim();
        currentPage = 1; 
        // Note: We don't clear the category filter here, so search is applied within the current category.
        // If the user selects a category and then searches, they search only that category.
        filterAndPaginateProducts();
    }
    
    // 🚀 NEW LISTENER: Listen for the custom event dispatched from the navbar component
    document.addEventListener('navbar-search', (e) => {
        handleSearch(e.detail.query);
        
        // Optional: Ensure the category filter is reset to 'all' when using the main search bar
        // to give a site-wide search experience.
        if (currentFilter !== 'all') {
            currentFilter = 'all';
            filterButtons.forEach(btn => btn.classList.remove('active'));
            // Find and activate the 'All Products' button if it exists
            const allBtn = document.querySelector('.filter-btn[data-filter="all"]');
            if (allBtn) allBtn.classList.add('active');
        }
    });
    ...
    // ... (renderProducts, renderPagination, and Cart functions remain the same) ...
    function renderProducts(productsToDisplay) {
        productsGrid.innerHTML = ''; 
        if (!productsGrid || productsToDisplay.length === 0) {
            if (productsGrid) {
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            currentSearchTerm = ''; // Clear text search when category filter changes
            if (searchInput) searchInput.value = '';
